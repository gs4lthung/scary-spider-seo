...
function extractHtmlFragment(value: string): string {
  let html = value.trim().replace(/^```(?:html)?\s*/i, "").replace(/\s*```$/, "");
  const articleMarker = html.search(/^ARTICLE HTML:\s*/im);
  if (articleMarker >= 0) {
    html = html.slice(articleMarker).replace(/^ARTICLE HTML:\s*/i, "");
    html = html.split(/^\s*(?:FAQS|INTERNAL LINK OPPORTUNITIES|EXTERNAL SOURCES TO VERIFY|EDITOR NOTES):\s*$/im)[0].trim();
  }
    ...
    <div className="rounded border border-border">
      ...
      {htmlImportOpen ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="html-import-title"
        >
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-3xl rounded-lg bg-background shadow-xl">
            <div className="flex items-center justify-between border-b border-border p-4">
              <div>
                <h2 id="html-import-title" className="text-sm font-semibold">Import HTML</h2>
                <p className="mt-1 text-xs text-muted-foreground">Paste the article HTML generated by Gemini or another writing tool.</p>
              </div>
              <button type="button" onClick={() => setHtmlImportOpen(false)} className="text-sm text-muted-foreground hover:text-foreground">
                Close
              </button>
            </div>
            <div className="p-4">
              <textarea
                ...
                rows={16}
                spellCheck={false}
                className="w-full resize-y rounded border border-border bg-card px-3 py-2 font-mono text-xs leading-5 focus:outline-none focus:ring-2 focus:ring-primary"
              />
              <div className="mt-3 flex justify-end gap-2">
                <button type="button" onClick={() => setHtmlImportOpen(false)} className="rounded border border-border px-3 py-2 text-sm font-medium hover:bg-secondary">
                  Cancel
                </button>
                <button type="button" onClick={importHtml} disabled={!htmlImport.trim()} className="rounded bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50">
