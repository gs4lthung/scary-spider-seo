...
**Media** (`app/media/[key]/route.ts`): serves R2 objects directly by key with a far-future immutable `Cache-Control` and `ETag`; uploads go through `app/admin/media-actions.ts`.
